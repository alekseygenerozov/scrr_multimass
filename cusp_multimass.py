...
class Cusp_Multimass(object):
    ...
    def __init__(self, gamma=[1.75], mbh_mass=4e6,
                 star_mass=[1.0], rh=2.0, mass_frac=[1.0]):
        r"""
        """
        self.gamma = gamma
        self.comps = len(self.gamma)
        self.mbh_mass = float(mbh_mass)
        self.star_mass = star_mass
        self.mass_frac = mass_frac/np.sum(mass_frac)
        self.rh = float(rh)
        self.gr_factor = 1.0
        self.mu = 1.0
    ...
    def total_number_of_stars(self, comp):
        r"""
        Number of stars within the radius of influence :math:`r_{\mathrm{h}}`
        """
        return self.total_stellar_mass(comp)/ self.star_mass[comp]

    def total_stellar_mass(self, comp):
        r"""
        Total mass within the radius of influence :math:`r_\mathrm{h}` [solar mass]

        TODO - Implement normalization
        """
        return self.mbh_mass*self.mu*self.mass_frac[comp]
    ...
    def stellar_mass(self, a, comp):
        ...
        a: float, array
            Semi-major axis [pc].
        """
        return self.total_stellar_mass(comp) * (a / self.rh) ** (3 - self.gamma[comp])
    ...
    def nu_mass_comp(self, a, j, comp):
        r"""
        ...
        """
        return self._nu_mass0(a, comp) * self._g(j, comp)
    ...
    def nu_mass(self, a, j):
        ...
        j: float, array
            Normalized angular momentum :math:`j = J/J_{\mathrm{c}} = \sqrt{1-e^2}`.
        """
        nu_mass=self._nu_mass0(a, 0) * self._g(j, 0)
        for i in range(1, self.comps):
            nu_mass+=self._nu_mass0(a, i) * self._g(j, i)
       	return nu_mass
    ...
    def nu_p1(self, a):
        ...
        """
        mass_term=self._nu_mass0(a, 0) * (3 - self.gamma[0]) / 2
        for i in range(1, self.comps):
            mass_term+=self._nu_mass0(a, i) * (3 - self.gamma[i]) / 2

        return self.nu_gr(a, 1.0) + mass_term

    def d_nu_p_comp(self, a, j, comp):
        r"""
    ...
    def d_nu_p(self, a, j):
        ...
        The derivative of :math:`\nu_\mathrm{p}` with respect to :math:`j`, defined to be positive
        """
        d1=self.d_nu_p_comp(a, j, 0)
        for i in range(1, self.comps):
            d1+=self.d_nu_p_comp(a, j, i)
        return d1

    def inverse_cumulative_a(self, x, comp):
        r"""
        ...
        """
        return x ** (1 / (3 - self.gamma[comp])) * self.rh

    def _nu_mass0(self, a, comp):
        r"""
        The frequency of the mass precession divided by :math:`g(j)`.
        """
        return -self.nu_r(a) * self.stellar_mass(a, comp) / self.mbh_mass
    ...
    def _g(self, j, comp):
        ...
        g(1) = (3-\gamma)/2
        """
        p1 = self._eval_legendre_inv(1 - self.gamma[comp], j)
        p2 = self._eval_legendre_inv(2 - self.gamma[comp], j)
        return - j ** (4 - self.gamma[comp]) / (1 - j ** 2) * (p1 - p2 / j)

    def _gp(self, j, comp):
        """
        dg(j)/dj
        """ 
        n2 = 2 - self.gamma[comp]
        p1 = self._eval_legendre_inv(1 - self.gamma[comp], j)
        p2 = self._eval_legendre_inv(2 - self.gamma[comp], j)
