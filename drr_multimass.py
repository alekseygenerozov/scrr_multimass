...
class DRR_Multimass(Cusp_Multimass):
    ...
    def __call__(self, l_max, neval=1e3, threads=1,
        ...
        for comp in range(self.comps):
            for i in pbar:
                self._drr_lnnp(*lnnp[i], comp, neval=neval, threads=threads)

        drr_tot=np.zeros(len(self.j))
        drr_err_tot=np.zeros(len(self.j))
        for comp in range(self.comps):
            drr = np.vstack([self._drr_lnnp(*lnnp, comp, neval=neval, threads=threads)[0]
                             for lnnp in lnnp])
            ...
            drr = drr.sum(axis=0)
            drr_tot+= drr
            drr_err = sqrt(sum(self._drr_lnnp(*lnnp, comp, neval=neval,
                                              threads=threads)[-1] ** 2 for lnnp in lnnp))
            drr_err_tot = (drr_err_tot**2.0+drr_err*2.0)**0.5

        return drr_tot, drr_err_tot

    def _drr_lnnp(self, l, n, n_p, comp, neval=1e3, threads=1):
        r"""
        ...
        try:
            drr, drr_err = self._drr_lnnp_cache[Res(l, n, n_p, comp, neval)]
        ...
        if threads > 1:
            ...
            def parallel_drr(pos, j_s, omega_s, seed_s):
                _results = [
                    self._drr(j, omega, (l, n, n_p), comp, neval=neval, seed=seed)
            ...
            results = [self._drr(j, omega, (l, n, n_p), comp, neval=neval, seed=seed)
                       for j, omega, seed in
                       zip(self.j, self.omega, self.seeds)]

            drr, drr_err = np.array(list(zip(*results)))

        self._drr_lnnp_cache[Res(l, n, n_p, comp, neval)] = (drr, drr_err)
